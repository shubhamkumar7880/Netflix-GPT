...
const VideoTitle = ({ title, overview, movieId }) => {
  ...
  return (
    <div className="absolute pt-[13%]  px-6 md:px-24 text-white bg-gradient-to-r from-black w-screen aspect-video">
      ...
      <div className="flex">
        <button
          className="flex mx-2 bg-white text-black py-1 md:py-2 px-4 md:px-6 hover:bg-opacity-80 text-xl rounded-lg mt-4 md:mt-0"
          onClick={() => {
            navigate(
              "/watch/" + movieId + (genre !== "" ? "?genre=" + genre : "")
            );
          }}
        >
          <svg
        ...
        <button className="flex mx-2 bg-gray-500 opacity-80 text-white py-1 md:py-2 px-4 md:px-6 hover:bg-opacity-80 text-xl rounded-lg mt-4 md:mt-0">
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
